...
FileOpLock* FileOpLock::CreateLock(const std::wstring& name, FileOpLock::UserCallback cb, HANDLE p_provided)
{
	FileOpLock* ret = new FileOpLock(cb);

	if (ret->BeginLock(name, p_provided))
	{
		return ret;
	}
	else
